...
// ─── User settings (which providers are allowed to run at all) ──────────────
// Persisted as plain JSON in userData, same pattern as the setup-complete
// flag below — this app has no other need for a dependency like electron-store.
const DEFAULT_PROVIDER_ENABLED = { claude: true, antigravity: true, codex: true, cursor: true };
let providerEnabled = { ...DEFAULT_PROVIDER_ENABLED };
...
function loadSettings() {
  try {
    ...
    providerEnabled = { ...DEFAULT_PROVIDER_ENABLED, ...(raw.providerEnabled || {}) };
  } catch (e) {
    // missing/corrupt file — keep defaults
  }
}

function saveSettings() {
  try {
    fs.writeFileSync(settingsPath(), JSON.stringify({ providerEnabled }));
  } catch (e) {
...
async function detectOtherProviders() {
  ...
  for (const p of OTHER_PROVIDERS) {
    if (!providerEnabled[p.id]) {
      result[p.id] = { state: "disabled", message: null };
      continue;
    }
...
function runAgyCommand(agyPath, augmentedEnv) {
  return new Promise((resolve) => {
    ...
    const doneTimeout = setTimeout(() => child.kill(), 62000);
    let accumulated = "";
...
function runCursorCommand(cursorPath, augmentedEnv) {
  return new Promise((resolve) => {
    ...
    const doneTimeout = setTimeout(() => child.kill(), 32000);
    let accumulated = "";
    child.stdout.on("data", (d) => (accumulated += d.toString()));
    child.on("close", () => {
      clearTimeout(timeout);
...
function runClaudeCommand(command, parseFn) {
  return new Promise((resolve) => {
    ...
    // Safety timeout - if nothing happens in 26s, fail gracefully. Must stay
    // comfortably above pty-wrapper.py's own internal hard cap (20s) plus the
    // doneTimeout kill below (23s) so a slow-but-legitimate cold start gets a
    // chance to finish and parse cleanly instead of being cut off here first.
...
async function fetchUsageAndStats() {
  if (!providerEnabled.claude) return null;
  if (!(await isOnline())) {
    return { error: "You're offline.", errorType: "offline" };
  }

  // The PTY-driven /usage capture times its own completion off idle
  // detection against a real interactive TUI — a slow cold start or a
  // redraw landing at the wrong moment can race that into a spurious
  // "Could not find usage data" even though Claude Code is installed and
  // signed in fine. That previously meant the user had to manually mash
  // Refresh until one attempt got lucky; retry it here automatically
  // instead, same withRetryOnError policy already used for the other
  // providers below.
  const usage = await withRetryOnError(() => runClaudeCommand("/usage", parseUsageOutput));
  ...
  const stats = await withRetryOnError(() => runClaudeCommand("/stats", parseStatsOutput));
...
async function updateTrayTitle() {
  ...
  // that provider just got toggled off in Settings there's a window before
  // the popup notices and switches its own tab away — fall back to the
  // first still-enabled provider for the badge rather than showing a
  // disabled provider's stale color/percentage in the tray.
  const effectiveId = providerEnabled[selectedProviderId] ? selectedProviderId : firstEnabledProvider();
  const color = PROVIDER_COLORS[effectiveId] || PROVIDER_COLORS.claude;
  const letter = PROVIDER_LETTERS[effectiveId] || "C";
  ...
  if (effectiveId === "antigravity" && providerStatus.antigravity?.state === "loggedIn") {
  ...
  if (effectiveId === "codex" && providerStatus.codex?.state === "loggedIn") {
  ...
  if (effectiveId === "cursor" && providerStatus.cursor?.state === "loggedIn") {
...
// Order matters here — it's the fallback order used whenever the currently
// selected/tray-badge provider turns out to be disabled.
const PROVIDER_ORDER = ["claude", "antigravity", "codex", "cursor"];
function firstEnabledProvider() {
  return PROVIDER_ORDER.find((id) => providerEnabled[id]) || "claude";
}
...
function broadcastUsageUpdate() {
  ...
  if (settingsWindow && !settingsWindow.isDestroyed()) {
    settingsWindow.webContents.send("usage-update", payload);
  }
}
...
// ─── Settings window ─────────────────────────────────────────────────────────
// Opened from the tray's right-click menu. Custom-styled BrowserWindow
// (matching popup.html's dark theme) rather than anything built from native
// OS menu items, since a native menu can't host toggles/status text/buttons.
...
// Each provider is a CLI that does its own interactive terminal login (OAuth
// device flow, browser handoff, whatever) — there's no web popup to drive
// here. Simplest reliable approach: hand off to the user's real system
// terminal (which has their normal login shell / PATH / rc files, unlike
// this packaged app's own bare-bones env) running the bare binary name, and
// let the CLI's own login flow run exactly as it would if the user typed it
// themselves.
const PROVIDER_LOGIN_CMD = { claude: "claude", antigravity: "agy", codex: "codex", cursor: "cursor-agent" };

function openProviderTerminal(id) {
  const cmd = PROVIDER_LOGIN_CMD[id];
  if (!cmd) return;

  if (process.platform === "darwin") {
    // AppleScript string built only from our own fixed command table above
    // (never renderer-supplied text), so no injection concern despite the
    ...
    spawn("cmd.exe", ["/c", "start", "cmd.exe", "/k", cmd], { shell: false });
  } else {
    // No single canonical terminal on Linux — try common ones in order,
    // same fallback list dynamic-island-native-adjacent tooling uses.
    const terminals = ["x-terminal-emulator", "gnome-terminal", "konsole", "xterm"];
    const tryNext = (i) => {
      if (i >= terminals.length) return;
      const term = terminals[i];
      const args = term === "gnome-terminal" || term === "konsole" ? ["--", "bash", "-c", `${cmd}; exec bash`] : ["-e", cmd];
...
ipcMain.handle("get-settings", () => ({ providerEnabled }));

ipcMain.handle("set-provider-enabled", async (_, id, enabled) => {
  if (!(id in providerEnabled)) return { providerEnabled };
  providerEnabled[id] = !!enabled;
  saveSettings();

  // Re-run detection/tray immediately rather than waiting up to 5 minutes
  // for the next poll — toggling a provider on should make it start
  // reporting right away, and toggling one off should clear its tab/badge
  // right away too.
  await updateTrayTitle();
  broadcastUsageUpdate();
  if (enabled) {
...
ipcMain.handle("open-provider-login", (_, id) => openProviderTerminal(id));
