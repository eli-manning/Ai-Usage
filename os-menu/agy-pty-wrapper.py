...
# A fully-authenticated agy keeps a background goroutine loop alive (quota/
# experiment/model-config refresh against Google's backend, observed
# retrying every ~1s even after this wrapper sends SIGTERM), and it doesn't
# reliably exit on SIGTERM once that's running — whether it's ignoring the
# signal or hung in its own graceful-shutdown path, a plain SIGTERM leaves
# it orphaned indefinitely (PPID 1, still holding live Google connections,
# observed days later). SIGKILL can't be caught/blocked/ignored, so escalate
# to it if SIGTERM hasn't reaped the process within a short grace period.
#
# The forked child also called os.setsid() to become its own session/
# process-group leader, so any subprocess agy spawns (a sandboxed helper, an
# MCP server, etc. — distinct from the same-process goroutine loop above)
# lands in that same group. Signaling just `pid` only ever killed agy's
# top-level process — any such subprocess survived it and was orphaned to
# init instead of being cleaned up. Use killpg to hit the whole group.
